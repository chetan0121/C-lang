...
#define tableMode 1     // 1 = On , 0 = Off

/* meanings :
   CTtype => Central tendency type
   mtype/Methodtype => types of Methods for grouped, ungrouped with Fi and raw data.
*/
...
//---------- Functions ---------
int YorN(){     //To get choice in yes OR no
    int opt;
    printf("\n1. Yes");
    printf("\n2. No");
    printf("\n\nEnter Option ->");
...
int getMeanType(){  //Choose th method of arithmetic mean
    int mType=0;
    printf("\n\nTypes of Mean :");
    printf("\n1. Direct Method");
    printf("\n2. Assumed Mean");
...
void sortingArray(float no[], int len) {    //Bubble sorting to sort array in Ascending order
    int i, j, isSorted;
    float temp;
    ...
    for(i=0; i<len-1; i++) {
        ...
        if(isSorted){   //Check if sorted or not
            break;
        }
    }
}

float MaxfromArray(float arr[], int len, short int *imax){  //To get max number from an array, with its index
    int i;
    float max = arr[0];
    for (i=1; i<len; i++){
        if (arr[i] > max){
...
char* needSpace(int target, int lmt){   //To dynamically generate Spaces for table
    int digit = (target==0 ? 1 : 0);
    // printf("\nTarget = %d",target);

    while(target>0){    //Count digits
        digit++;
        target = target/10;
    }
    // printf("\nDiff = %d",diff);
    ...
    int i=0;
    int spaceCount = (lmt<digit)?(digit-lmt):(0);
    char *space = (char*) malloc((spaceCount+1)*sizeof(char));
...
int countMax(float target[], int len, short int *imax){ //To check that if the Mode is One or more (after getting their frequency)
    int count=0, i;
...
int get_input(int CTtype) {   //Get input from user as preferred
    ...
    if(opt==1){
        ...
        for(i=0; i<len; i++){
            ...
            if(sign!=' ' && sign!='-'){
                printf("\nInvalid sign. Please refer to the example provided above.\n");
                goto restart;
            }
            ...
            if(class[i][0]<0 || class[i][1]<0){
                printf("\nCalculations for negative values are not currently available.");
                goto restart;
            }
            while(getchar()!='\n');
        }
        ...
        for(i=0; i<len; i++){
            ...
            printf("No%2d =>",i+1);
            if(scanf("%f",&fi[i])==0){
                printf("Invalid input for frequency.\n\n");
                goto restart2;
            }
            if(fi[i]<0){
        ...
        if(opt==1){
            ...
            for(i=0; i<len; i++){
                ...
                if(xi[i]<0){
                    printf("Calculations for negative values are not currently available.");
                    goto restart3;
                }
            }
...
int findMean(int n, float classRange[][2], float fi[], float xi[], int mtype){  //Find mean
    int i, j, meanType=0;
    float ans=0;
    ...
    if(mtype==1){    //method type == Class and fi
    ...
    else if(mtype==2){    //Method type ==> Ungrouped (xi & fi)
        float Sfi=0;
        ...
        if(meanType==1){
            ...
            if(tableMode){
                ...
                space2 = needSpace(MaxfromArray(fi,n, NULL), 6);
    
                printf("\n\n%s--Xi--%s  --Fi--%s  --Fixi--",space1,space1,space2);
                for (i = 0; i < n; i++){
            ...
            if(tableMode){
                space1 = needSpace(MaxfromArray(xi,n, NULL), 6);
                space2 = needSpace(MaxfromArray(fi,n, NULL), 6);
                space3 = needSpace(MaxfromArray(di,n, NULL), 6);
    
                printf("\n\n%s--Xi--%s  --Fi--%s  --di--%s  --Fidi--",space1,space1,space2,space3);
                for (i = 0; i < n; i++){
            ...
            if(tableMode){
                space1 = needSpace(MaxfromArray(xi,n, NULL), 6);
                space2 = needSpace(MaxfromArray(fi,n, NULL), 6);
                space3 = needSpace(MaxfromArray(ui,n, NULL), 6);
    
                printf("\n\n%s--Xi--%s  --Fi--%s  --ui--%s  --Fiui--",space1,space1,space2,space3);
                for (i = 0; i < n; i++){
...
int findMedian(int n, float ClassInterval[][2], float fi[], float xi[], int MethodType){
    ...
    if(MethodType==1){
        ...
        for(i=0; i<n; i++){ //to get cfi and Summation Fi
            if(i==0){
                Cfi[0] = fi[0];
            }
            else {
        ...
        if(tableMode){
            ...
            for(i=0; i<n; i++){
                if(Cfi[i]==cf){
                    printf("\n %4.f-%-4.f     %4.f     %7.f  <- Cf", ClassInterval[i][0], ClassInterval[i][1], fi[i], Cfi[i]);
                }
                else {
                    printf("\n %4.f-%-4.f     %4.f     %7.f", ClassInterval[i][0], ClassInterval[i][1], fi[i], Cfi[i]);
                }
            }
        }
        ...
        for(i=0; i<n-1; i++)
        {
            for(j=0; j<n-i-1; j++){       //Arranging Data(Xi) in ascending order with fi
                if(xi[j]>xi[j+1]){
                    temp = xi[j];
                    xi[j] = xi[j+1];
                    xi[j+1] = temp; 
        ...
        for(i=0; i<n; i++){     //Finding Cf from Cfi
            if(Cfi[i] > (Sfi+1)/2){
                cf = Cfi[i];
                ans = xi[i];
                break;
        ...
        if(tableMode){
            ...
            printf("\n\n%s--Xi--%s   --Fi--%s   --Cfi--", space1, space1,space2);
            for(i=0; i<n; i++){
                if(Cfi[i] == cf){
